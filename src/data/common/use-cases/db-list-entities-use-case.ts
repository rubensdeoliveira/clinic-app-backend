...
export class DbListEntitiesUseCase<EntityType = object> implements ListEntitiesUseCase<EntityType> {
  ...
  async list ({ textToSearch, page = 1, recordsPerPage = 15, orderDirection = OrderDirection.ASC, orderColumn, filters }: ListEntitiesDTO): Promise<ListEntityModel<EntityType>> {
    const recordCount = await this.countEntitiesRepository.count(textToSearch, filters) as number
    if (recordCount === 0) {
      return {
        data: [],
        last_page: 0,
