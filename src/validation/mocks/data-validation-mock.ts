...
export const addFieldValue = (record: object, field: string, value: any = faker.datatype.uuid()): object => ({
  ...record,
  [field]: value
})
...
export const mockDifferentFieldValue = (fields: string []): object => {
  const record = {}
  fields.forEach((field) => { record[field] = faker.datatype.uuid() })
  return record
}
...
export const mockRangeValue = (field: string, { min, max }: RangeOptions): object => ({
  [field]: faker.datatype.number({ min, max })
})
